...
logger.info(`📡 WebSocket server pokrenut na ws://localhost:${PORT}`);
...
app.listen(8082, () => {
  logger.info('📦 Express server sluša na http://localhost:8082');
});
...
wss.on('connection', async (ws, req) => {
  ...
  clients.push(ws);
  logger.info('✅ Povezan novi korisnik.');
  ...
  ws.on('close', () => {
    ...
    logger.info(`❌ Korisnik se odspojio.`);
  });
});
